...
export const useStudentData = () => {
  ...
  const submitPHQTest = async (answers: Record<number, number>) => {
    ...
    const { data, error } = await supabase.from('phq_tests').insert({ 
      student_id: studentData.id, 
      score, 
      answers, 
      severity_level: severityLevel,
      test_date: new Date().toISOString() // Explicitly set the test_date
    }).select().single();
  ...
  const submitGAD7Test = async (answers: Record<number, number>) => {
    if (!studentData || !user) { // Also check for user
      console.error("User or student data not found!");
      return { error: { message: "Student not found" }};
    }

    // Add these console logs for debugging
    console.log("Submitting GAD-7 test for student ID:", studentData.id);
    ...
    const { data, error } = await supabase.from('gad_7_tests').insert({
      student_id: studentData.id,
      score,
      answers,
      severity_level: severityLevel,
    ...
    if (error) {
        console.error("Error saving GAD-7 test:", error);
    }
